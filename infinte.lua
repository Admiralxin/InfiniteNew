...
local function randomDelay(min, max)
    return rand:NextNumber(min or 0.1, max or 0.25)  -- Extended delays for more randomness
end
    ...
    while Toggles.Killaura.Value and ao do
        ...
        if alive() and not mounted() and X and not table.find(bl, aZ) then
            ...
            for _, gx in ipairs(skillList) do
                ...
                if rand:NextNumber() < 0.2 then  -- Decreased chance of skipping (to mimic human hesitation)
                ...
                local randomAttackInterval = rand:NextNumber(0.4, 0.75)  -- Even longer random intervals
                ...
                if now - gx.LastUsed >= cooldown and now - lastGlobalAttack >= globalCooldown + attackLag then
                    if gy ~= 'Heal' and ge <= gA and a2 and a2.Value > 0 then
                        simulateLookAt(Z)
                        task.wait(randomDelay(0.1, 0.2))  -- More randomization in skill activation
                        ...
                        local fireDelay = rand:NextNumber(0.5, 1.0)  -- Longer intervals between `FireServer` calls
                        task.wait(fireDelay)
        ...
        task.wait(rand:NextNumber(0.3, 0.5))  -- More extended idle time
    end
end)
